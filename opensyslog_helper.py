...
class OpensyslogHelper:
    ...
    def setup_lookup_csv_file(self, data=None):
        self.mac_to_name_lookup_dict = {}
        try:
                ...
                if file_name is not None and len(file_name) > 0:
                    ...
                    if os.path.exists(file_name_with_path):
                        with open(file_name_with_path, "r") as csv_file:
                            ...
                            if data:
                                self.add_data(data, file_name_with_path)
    ...
    def add_data(self,data, file):
        if data[1] not in self.mac_to_name_lookup_dict:
            self.mac_to_name_lookup_dict[data[1]] = data[2]
            with open(file, mode='a', newline='') as csvfile:
                csv_writer = csv.writer(csvfile)
                csv_writer.writerow(data)
            print(f"Successfully loaded CSV file with {len(self.mac_to_name_lookup_dict)} rows")        
        else:
            self.print(self.log_level_info, "Device already present") 
